...
plot_server <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    custom_labels <- c("3" = "Very Barbarous", "2" = "Relatively Neutral", "1" = "Relatively Civil")
    ...
    output$interactive_plot <- renderGirafe({
      ...
      plot <- ggplot() +
        ...
        labs(
          title = 'Distribution of Connotations of "Native" \n on Civil-Barbarous Dichotomy',
          x = 'Note: the more rightward, the more civil is the connotation of "Native"',
          y = ""
        ) +
        theme(
    ...
    output$line_plot <- renderGirafe({
      ...
      df_k <- df %>%
        ...
        select(Filename, P1_scaled, P2_scaled, P1_scaled)
      ...
      merged_df <- df %>%
        ...
        select(Filename, P1, P2, P1, Title, Author, Year, cluster_P1, cluster_P2, cluster_P1)
