...
plot_server <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    custom_labels <- c("3" = "Extremely Dark", "2" = "Relatively Neutral", "1" = "More Light-inclined")
    ...
    output$interactive_plot <- renderGirafe({
      ...
      annotations <- lines_df %>%
        ...
        x_pos = case_when(
          Filename == "A73849" ~ P3 - 0.02,
          Filename == "A19313" ~ P3 + 0.08,
          Filename == "A14803" ~ P3 + 0.08
        ),
        y_pos = case_when(
          Filename == "A73849" ~ 1.6,
          Filename == "A19313" ~ 1.3,
      ...
      merged_df$cluster_P3 <- factor(merged_df$cluster_P3, levels = c(3, 2, 1))
      ...
      plot <- ggplot() +
        ...
        labs(
          title = 'Distribution of Connotations of "Virginia" \n on Light-Dark Dichotomy',
          x = 'Note: the more rightward, the more light is the connotation of "Virginia"',
          y = ""
        ) +
        theme(
