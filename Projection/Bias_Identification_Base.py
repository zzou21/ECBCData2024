...
def get_word_embedding(chunks, tokenizer, model, word):
    ...
    for chunk in chunks:
        ...
        for token in re.split(r'\W+', chunk):
            if (token in substitute_word(word)) or (token == word):
                in_set = True
...
# Construct bias axes
def construct_bias_axes(category_embeddings, categories):
    faith_bias_axis = category_embeddings[categories[0]] - category_embeddings[categories[1]]
    education_bias_axis = category_embeddings[categories[2]] - category_embeddings[categories[3]]
    clothes_bias_axis = category_embeddings[categories[4]] - category_embeddings[categories[5]]
...
def substitute_word(word):
    ...
    #Below is the attempt to automate the process of finding substitute words. Ultimately it was not used, and we end up manually providing a list of subsitute words.
...
def project_onto_bias_axis(word, embeddings, a1, a2, a3):
    axes = [a1, a2, a3]
    projections = []
    for bias_axis in axes:
        ...
        if word in embeddings:
            ...
            for substitute in substitute_word(word):
...
# Main function
def main(categories_json, document_path, model_name, keyword, base_dir):
    # Load categories
    categories = load_categories(categories_json)
    category_name = [cat for cat, dict in categories.items()]
    ...
    # Construct bias axes
    a1, a2, a3 = construct_bias_axes(category_embeddings, category_name)
...
document_directory = os.path.join('/Users/lucasma/Downloads/AllVA_punc')
