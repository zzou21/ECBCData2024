...
plot_server <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    output$interactive_plot <- renderGirafe({
      ...
      merged_df$cluster_P1 <- factor(merged_df$cluster_P1, levels = c(1, 3, 2))
      
      # Define custom labels and colors for the clusters
      custom_labels <- c("1" = "London-inclined", "3" = "Relatively Neutral", "2" = "Extreme plantation")
      ...
      plot <- ggplot() +
        ...
        labs(
          ...
          x = 'Note: the more rightward, the more plantation is the connotation of "Virginia"',
          y = ""
        ) +
        theme(
