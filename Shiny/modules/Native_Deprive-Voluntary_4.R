...
plot_server <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    custom_labels <- c("2" = "Very Voluntary", "3" = "Relatively Neutral", "1" = "Relatively Deprived")
    ...
    output$interactive_plot <- renderGirafe({
      ...
      annotations <- lines_df %>%
        ...
        x_pos = case_when(
          ...
          Filename == "A19313" ~ P3 + 0.12,
          Filename == "A14803" ~ P3 - 0.06
        ),
        y_pos = case_when(
          Filename == "A73849" ~ 0.8,
          Filename == "A19313" ~ 0.7,
      ...
      merged_df$cluster_P3 <- factor(merged_df$cluster_P3, levels = c(2, 3, 1))
      ...
      plot <- ggplot() +
        ...
        labs(
          title = 'Distribution of Connotations of "Native" \n on Deprived-Voluntary Dichotomy',
          x = 'Note: the more rightward, the more deprived is the connotation of "Native"',
          y = ""
        ) +
        theme(
